...
class x extends c {
  constructor(t, e, i) {
    super(t, e, i, "function"), this.$button = document.createElement("button"), this.$button.appendChild(this.$name), this.$widget.appendChild(this.$button), this.$button.addEventListener("click", (l) => {
...
class B extends c {
  constructor(t, e, i, l, r, o) {
    super(t, e, i, "number"), this._initInput(), this.min(l), this.max(r);
    const d = o !== void 0;
    this.step(d ? o : this._getImplicitStep(), d), this.updateDisplay();
  }
  decimals(t) {
  ...
  _initInput() {
    ...
    const t = () => {
      ...
      isNaN(a) || (this._snapClampSetValue(a + s), this.$input.value = this.getValue());
    }, i = (s) => {
    ...
    let r = !1, o, d, p, h, u;
    const m = 5, v = (s) => {
      o = s.clientX, d = p = s.clientY, r = !0, h = this.getValue(), u = 0, window.addEventListener("mousemove", f), window.addEventListener("mouseup", b);
    }, f = (s) => {
      if (r) {
        const a = s.clientX - o, g = s.clientY - d;
        Math.abs(g) > m ? (s.preventDefault(), this.$input.blur(), r = !1, this._setDraggingStyle(!0, "vertical")) : Math.abs(a) > m && b();
      }
      if (!r) {
        const a = s.clientY - p;
        u -= a * this._step * this._arrowKeyMultiplier(s), h + u > this._max ? u = this._max - h : h + u < this._min && (u = this._min - h), this._snapClampSetValue(h + u);
      }
      p = s.clientY;
    }, b = () => {
      this._setDraggingStyle(!1, "vertical"), this._callOnFinishChange(), window.removeEventListener("mousemove", f), window.removeEventListener("mouseup", b);
    }, w = () => {
      this._inputFocused = !0;
    }, _ = () => {
      this._inputFocused = !1, this.updateDisplay(), this._callOnFinishChange();
    };
    this.$input.addEventListener("input", t), this.$input.addEventListener("keydown", i), this.$input.addEventListener("wheel", l, { passive: !1 }), this.$input.addEventListener("mousedown", v), this.$input.addEventListener("focus", w), this.$input.addEventListener("blur", _);
  }
  _initSlider() {
    ...
    const t = (s, a, g, $, k) => (s - a) / (g - a) * (k - $) + $, e = (s) => {
      const a = this.$slider.getBoundingClientRect();
      let g = t(s, a.left, a.right, this._min, this._max);
      this._snapClampSetValue(g);
    }, i = (s) => {
      this._setDraggingStyle(!0), e(s.clientX), window.addEventListener("mousemove", l), window.addEventListener("mouseup", r);
    }, l = (s) => {
      e(s.clientX);
    }, r = () => {
      this._callOnFinishChange(), this._setDraggingStyle(!1), window.removeEventListener("mousemove", l), window.removeEventListener("mouseup", r);
    };
    let o = !1, d, p;
    const h = (s) => {
      s.preventDefault(), this._setDraggingStyle(!0), e(s.touches[0].clientX), o = !1;
    }, u = (s) => {
      s.touches.length > 1 || (this._hasScrollBar ? (d = s.touches[0].clientX, p = s.touches[0].clientY, o = !0) : h(s), window.addEventListener("touchmove", m, { passive: !1 }), window.addEventListener("touchend", v));
    }, m = (s) => {
      if (o) {
        const a = s.touches[0].clientX - d, g = s.touches[0].clientY - p;
        Math.abs(a) > Math.abs(g) ? h(s) : (window.removeEventListener("touchmove", m), window.removeEventListener("touchend", v));
      } else
        s.preventDefault(), e(s.touches[0].clientX);
    }, v = () => {
      this._callOnFinishChange(), this._setDraggingStyle(!1), window.removeEventListener("touchmove", m), window.removeEventListener("touchend", v);
    }, f = this._callOnFinishChange.bind(this), b = 400;
    let w;
    const _ = (s) => {
      if (Math.abs(s.deltaX) < Math.abs(s.deltaY) && this._hasScrollBar)
        return;
      s.preventDefault();
      const a = this._normalizeMouseWheel(s) * this._step;
      this._snapClampSetValue(this.getValue() + a), this.$input.value = this.getValue(), clearTimeout(w), w = setTimeout(f, b);
    };
    this.$slider.addEventListener("mousedown", i), this.$slider.addEventListener("touchstart", u, { passive: !1 }), this.$slider.addEventListener("wheel", _, { passive: !1 });
  }
...
class C {
  ...
  constructor({
    ...
    if (this.parent = t, this.root = t ? t.root : this, this.children = [], this.controllers = [], this.folders = [], this._closed = !1, this._hidden = !1, this.domElement = document.createElement("div"), this.domElement.classList.add("lil-gui"), this.$title = document.createElement("div"), this.$title.classList.add("title"), this.$title.setAttribute("role", "button"), this.$title.setAttribute("aria-expanded", !0), this.$title.setAttribute("tabindex", 0), this.$title.addEventListener("click", () => this.openAnimated(this._closed)), this.$title.addEventListener("keydown", (h) => {
      (h.code === "Enter" || h.code === "Space") && (h.preventDefault(), this.$title.click());
    ...
    this.domElement.classList.add("root"), p && this.domElement.classList.add("allow-touch-styles"), !E && d && (Y(H), E = !0), i ? i.appendChild(this.domElement) : e && (this.domElement.classList.add("autoPlace"), document.body.appendChild(this.domElement)), l && this.domElement.style.setProperty("--width", l + "px"), this._closeFolders = o;
  }
  /**
   ...
   */
  addFolder(t) {
    const e = new C({ parent: this, title: t });
  ...
  load(t, e = !0) {
    return t.controllers && this.controllers.forEach((i) => {
      i instanceof x || i._name in t.controllers && i.load(t.controllers[i._name]);
...
export {
  S as BooleanController,
  O as ColorController,
  c as Controller,
  x as FunctionController,
  C as GUI,
  B as NumberController,
  z as OptionController,
